...
def trainsdbz(x):
    ...
    #return numpy.clip(x * 0.4 - 30,0,70)
    return x * 0.4 - 30
def trans_to_rainrate(x):
    """
    Convert dbZ to mm/h
    """


    p = numpy.power(10,x/10)/200
    p = numpy.clip(p,-100000,100000)
    
    rainrate = numpy.power(numpy.power(p,2),1/3)
    return rainrate
...
class Get_tager_sample_h5npy(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x = torch.clip(x,0,20.0)
        x = einops.rearrange(x," w h t -> t w h")
        # pwt_x_4 = pwt_x_4.to(dtype = x.dtype)
